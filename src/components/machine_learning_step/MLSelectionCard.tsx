...
export default function MLSelectionCard(properties: MLSelectionCardProperties) {
    return (
        <label className="group cursor-pointer relative">
            <input className="peer sr-only" name="model-selection" type="radio" value={properties.value} />
            <div className="h-full rounded-xl border-2 border-slate-200 bg-white p-5 transition-all duration-200 peer-checked:border-primary peer-checked:bg-blue-50/50 peer-checked:shadow-lg peer-checked:shadow-primary/20 hover:border-primary/50 flex flex-col justify-between">
                <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        <span className="material-symbols-outlined rounded-full bg-blue-100 p-2 text-primary">{properties.icon}</span>
                        <div className="text-primary opacity-0 peer-checked:opacity-100 transition-opacity absolute top-5 right-5">
                            <span className="material-symbols-outlined text-[24px]">check_circle</span>
                        </div>
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 mb-1 group-hover:text-primary transition-colors">{properties.name}</h3>
                        <p className="text-sm text-slate-500 leading-normal">{properties.desc}</p>
                    </div>
                </div>
